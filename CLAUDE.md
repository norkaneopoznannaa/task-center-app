...
## 19. ПЛАН UI/UX ДОРАБОТОК v1.6.0 (2026-02-02)

### Результаты аудита UI/UX

**Общая оценка: 6.5/10**

Приложение функционально, но имеет критические проблемы с консистентностью цветов и визуальной перегруженностью.

### 🔴 КРИТИЧЕСКИЕ ПРОБЛЕМЫ

#### 1. Несогласованность цветов категорий

**Проблема:** Категория РЭМД отображается разными цветами в разных местах:

| Место | Цвет | Файл |
|-------|------|------|
| types/index.ts | 🔵 Синий `#60a5fa` | `app/src/types/index.ts:47` |
| Sidebar (фильтры) | 🔵 Синий `#60a5fa` | Использует CATEGORY_COLORS |
| DailyPlan.css | 🔴 Красный `#f87171` | `app/src/components/DailyPlan.css:249-252` |

**Полная таблица несоответствий:**

| Категория | types/index.ts | DailyPlan.css | Должно быть |
|-----------|----------------|---------------|-------------|
| РЭМД | 🔵 `#60a5fa` | 🔴 `#f87171` | 🔵 Синий |
| КУ ФЭР | 🟡 `#fbbf24` | 🔵 `#60a5fa` | 🟡 Желтый |
| Общие | ⚪ `#737373` | 🟢 `#4ade80` | ⚪ Серый |
| Авто | 🟢 `#4ade80` | ⚪ `#9ca3af` | 🟢 Зеленый |

#### 2. Визуальная перегруженность

- 7+ визуальных элементов на строку задачи
- Не соблюдается правило 60-30-10
- ~40% акцентных цветов вместо рекомендуемых 10%

#### 3. Эмодзи вместо SVG иконок в TaskRow

- Кнопки действий используют эмодзи (▶️, ⏹️, ✏️, 🗑️)
- Разное отображение на Windows/Mac/Linux

### 🟡 СРЕДНИЕ ПРОБЛЕМЫ

1. Мелкий шрифт в метках (10px, рекомендуется 12px+)
2. Избыточный UPPERCASE в badges
3. Case sensitivity в фильтрах категорий

---

### ПЛАН ИСПРАВЛЕНИЙ (5 фаз)

#### Phase 1: Исправление цветов категорий (КРИТИЧНО)
**Файлы:** `app/src/components/DailyPlan.css`
**Сложность:** Низкая | **Влияние:** Высокое

```css
/* ИСПРАВИТЬ в DailyPlan.css:249-268 */
.category-tag.remd {
  background: rgba(96, 165, 250, 0.2);
  color: #60a5fa;  /* Синий, как в types/index.ts */
}
...
**Также исправить маппинг в DailyPlan.tsx:334-342:**
```typescript
const getCategoryClass = (category: string) => {
  const map: Record<string, string> = {
    'РЭМД': 'remd',
    'КУ ФЭР': 'kufer',
    'Общие': 'common',  // Было 'общие'
    'Авто': 'auto',     // Было 'авто'
  };
  return map[category] || map[category.toLowerCase()] || 'default';
};
```

#### Phase 2: SVG иконки в TaskRow
**Файлы:** `app/src/components/TaskRow.tsx`
**Сложность:** Средняя | **Влияние:** Среднее

- Заменить эмодзи ▶️/⏹️/✏️/🗑️/📋/📅 на SVG из `icons/index.tsx`
- Добавить недостающие иконки: Duplicate, Calendar
- Унифицировать размер иконок (16x16)

#### Phase 3: Улучшение типографики
**Файлы:** `app/src/styles/index.css`, компоненты
**Сложность:** Низкая | **Влияние:** Среднее

```css
/* Добавить в index.css */
:root {
  --font-size-xs: 11px;   /* Было 10px */
  --font-size-sm: 12px;   /* Было 11px */
}

/* Убрать uppercase из меток */
.badge {
...
#### Phase 4: Упрощение строки задачи
**Файлы:** `app/src/components/TaskRow.tsx`, `TaskRow.css`
**Сложность:** Высокая | **Влияние:** Высокое

**Текущее состояние:**
```
[CRITICAL] [РЭМД] Название задачи [В работе ▼] [EGISZREMD-123] [2ч] [24.01] [▶️✏️🗑️]
```

**Целевое состояние:**
```
🔴 Название задачи                    [В работе ▼] EGISZREMD-123  2ч  [▶]
   РЭМД · Через 4 дня
```

- Показывать приоритет только цветной полоской слева
- Категорию и дедлайн — во второй строке, мелким шрифтом
- Действия — показывать по hover

#### Phase 5: Ограничение цветовой палитры
**Файлы:** `app/src/styles/index.css`
**Сложность:** Средняя | **Влияние:** Среднее

**Целевая палитра (6 цветов):**

| Роль | Цвет | CSS Variable | Использование |
|------|------|--------------|---------------|
| Primary | `#D97757` | `--accent` | Кнопки, активные элементы |
| Success | `#4ade80` | `--status-done` | Завершено |
| Warning | `#fbbf24` | `--status-progress` | В работе |
| Error | `#f87171` | `--status-blocked` | Просрочено, блок |
| Info | `#60a5fa` | `--status-new` | Новая, информация |
| Neutral | `#737373` | `--text-muted` | Бэклог, отключено |
...
|---|--------|-------|-----------|--------|
| 1 | Исправить цвета категорий в DailyPlan | DailyPlan.css, DailyPlan.tsx | Низкая | ⏳ Запланировано |
| 2 | Заменить эмодзи на SVG в TaskRow | TaskRow.tsx | Средняя | ⏳ Запланировано |
| 3 | Увеличить шрифты меток | index.css | Низкая | ⏳ Запланировано |
| 4 | Упростить отображение TaskRow | TaskRow.tsx, TaskRow.css | Высокая | ⏳ Запланировано |
| 5 | Оптимизировать цветовую палитру | index.css | Средняя | ⏳ Запланировано |

---

### Версия после выполнения: v1.6.0

### Источники best practices:
- [UI Color Palette 2025 - IxDF](https://www.interaction-design.org/literature/article/ui-color-palette)
- [Design Consistency Guide - UXPin](https://www.uxpin.com/studio/blog/guide-design-consistency-best-practices-ui-ux-designers/)
